...
program main2
  ...
  ! dfdy
  call test_func(x, y+h, z, fh)
  dfdy_fd = (fh - f)/h
  errory_fd = abs(1 - dfdy_fd/dfdy_an)

  ! dfdz
  call test_func(x, y, z+h, fh)
  dfdz_fd = (fh - f)/h
  ...
  ! dfdy
  call test_func_complex(complex(x,0.0), complex(y,h), complex(z,0.0), fhc)
  dfdy_cs = imag(fhc)/h
  errory_cs = abs(1 - dfdy_cs/dfdy_an)

  ! dfdz
  call test_func_complex(complex(x,0.0), complex(y,0.0), complex(z,h), fhc)
  dfdz_cs = imag(fhc)/h
