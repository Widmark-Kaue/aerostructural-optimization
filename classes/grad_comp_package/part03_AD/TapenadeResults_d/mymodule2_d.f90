...
CONTAINS
!  Differentiation of test_func in forward (tangent) mode:
!   variations   of useful results: f
!   with respect to varying inputs: x y z
!   RW status of diff variables: f:out x:in y:in z:in
  SUBROUTINE TEST_FUNC_D(x, xd, y, yd, z, zd, f, fd)
    IMPLICIT NONE
    ...
    REAL :: temp
    temp = COS(x)
    ad = temp*yd - y*SIN(x)*xd + zd
    a = y*temp + z
    fd = 2*a*ad - 2.0*xd/x**2
    f = a**2 + 2.0/x
  END SUBROUTINE TEST_FUNC_D
