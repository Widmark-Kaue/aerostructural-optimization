...
#%% 5.6 Comparison with eliptical lift distribution
...
plt.savefig(imagdir / f'q5.6_2_comp_opt_x_eliptical.{format}', 
            dpi=dpi, bbox_inches='tight') if saveflag else None
plt.show()

#%% 5.6 Plot margins of FEM nodes
...
plt.savefig(imagdir / f'q5.6_3_margins.{format}', 
            dpi=dpi, bbox_inches='tight') if saveflag else None
#%% 5.6 Weight Fractions
...
plt.savefig(imagdir / f'q5.6_4_weight_fractions.{format}', 
            dpi=dpi, bbox_inches='tight') if saveflag else None
...
# Inset zoom for FB (Use linear plot)
axins3 = inset_axes(ax3, width="40%", height="40%", loc="upper right")
axins3.plot(fb_hist, 'k')

y1 = np.min(fb_hist[x1:x2])
y2 = np.max(fb_hist[x1:x2])
...
plt.savefig(imagdir / f'q5.6_5_desVars_obj_hist.{format}', 
            dpi=dpi, bbox_inches='tight') if saveflag else None
plt.show()
...
plt.tight_layout()
plt.savefig(imagdir / f'q5.6_5_constraint_hist.{format}', 
            dpi=dpi, bbox_inches='tight') if saveflag else None
